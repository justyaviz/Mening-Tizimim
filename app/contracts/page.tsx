    ...
    <div className="pageWrap">
      ...
      <Modal open={open} title={data.contracts.some((c) => c.id === editing?.id) ? "Shartnomani tahrirlash" : "Yangi shartnoma"} subtitle="Shartnoma bo‘yicha asosiy nazorat ma’lumotlari." onClose={() => setOpen(false)}>
        {editing && <form className="formGrid" onSubmit={save}>
          ...
          <label className="field"><span>Mijoz</span><input list="contract-client-options" value={editing.client} onChange={(e) => setEditing({ ...editing, client: e.target.value })} /><datalist id="contract-client-options">{data.clients.map((client) => <option key={client.id} value={client.name} />)}</datalist></label>
          <label className="field"><span>Loyiha</span><input list="contract-project-options" value={editing.project} onChange={(e) => setEditing({ ...editing, project: e.target.value })} /><datalist id="contract-project-options">{data.projects.map((project) => <option key={project.id} value={project.name} />)}</datalist></label>
