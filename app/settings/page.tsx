  ...
  return (
    <div className="pageWrap">
      ...
      <section className="settingsStack">
        ...
        <article className="settingsCard cardLike"><div><Download size={19}/><div><strong>Backup yuklab olish</strong><p>Loyiha, mijoz, shartnoma, moliya va vazifalarni JSON backup sifatida saqlang.</p></div></div><button className="secondaryButton" onClick={exportData}>Yuklab olish</button></article>
        <article className="settingsCard cardLike"><div><RotateCcw size={19}/><div><strong>Demo ma’lumotlarni tiklash</strong><p>Hozirgi workspace ma’lumotlarini boshlang‘ich v0.3 demo holatiga qaytaradi.</p></div></div><button className="secondaryButton" onClick={() => window.confirm("Demo ma’lumotlarni tiklaysizmi?") && resetDemo()}>Tiklash</button></article>
      </section>
    </div>
  );
}
